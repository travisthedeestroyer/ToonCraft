...
export interface Scene {
  ...
  musicMood?: string; // Mood for the background music
  bgMusicUrl?: string; // Background music for this scene
  isReady?: boolean; // Fully generated (Audio + Visual)
  isGenerating?: boolean; // Currently being worked on
  fallbackToImage?: boolean; // If video failed
}
...
export type GenerationProgress = {
  status: 'scripting' | 'visuals' | 'audio' | 'ready' | 'progressive';
  currentScene: number;
  totalScenes: number;
  message: string;
  scenesReady: number;
